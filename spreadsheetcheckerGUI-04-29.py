...
##### v. 1.1
##### 2014. armen karamian
##### GUI that combines spreadsheet checking and entry
##### NOTE: Most button are created with a method and assigned another method to execute when pressed.

import Tkinter as tk
import spreadsheetChecker20140401 as sc																					### this script uses functions from the spreadsheetchecker script
import spreadsheetinserttest20140401 as si																				### this script uses functions from the spreadsheet insert script	
import NbcFileClass as nbc																								### special class created for nbc files
import PyQTMovie2 as qt																									### creates the NBCFile Object 
import string
import re

class Window(tk.Frame):																									# this is the class for the second window that shows the slate.
	def __init__(self, parent):
		tk.Frame.__init__(self,parent)
		self.image = tk.PhotoImage(file='slate.gif')																	### this is the filename for the slate image that is displayed
...
class Application(tk.Frame):
	def __init__(self, master=None):
		...
		self.OUTPUT_FILENAME = '/Volumes/dam/_NBCU/EntryScriptOutput/output.txt'								#### !!!!!! FILE WHERE OUTPUT FOR WINDOWS SCRIPT GOES ######
		self.EXCEL_FILE = '/Volumes/fs3/encoding/AssetManagement/NBCU_Features_Series_MASTER.xls'				#### !!!!!! PATH FOR EXCEL FILE !!!!! #######
		self.NBC_FILE_PATH = '/Volumes/fs3/encoding/AssetManagement/nbc_incoming/'								#### !!!!!! PATH FOR INCOMING FILES !!!!!! #######
		self.grid()
		...
		self.clientID.set('UNIVERSAL')

	
	def updateDirectory(self):																				#allow custom directory to be used
		tempPath = self.newFilePath.get().strip()
		if not (tempPath.endswith('/')):
			tempPath += '/'
		self.refreshList(tempPath)
	...
	def getInfo(self):																									#get info about selected item in listboxes
		...
		selectedFilePath = self.NBC_FILE_PATH+selectedFilename														#concat filepath with filename
	...
	def submitInfo(self):																								#read from variables and put into output text doc in append/binary mode.
		output = open(self.OUTPUT_FILENAME,'ab')																		#append to new text file created when app is opened
		output.write(self.titlevar.get()+'\r\n')																		# '\r\n' is used to make the line breaks windows compatible
		...
		output.write(self.clientID.get()+'\r\n')
		output.write(string.upper(self.danumbervar.get())+'\r\n')
		output.write(self.filenamevar.get()+'\r\n')
		output.close()
		
		self.titlevar.set('')																							#reset variables to blank
	...
	def createEntryWindows(self,file=None):																				#set fields to variables
		if (file != None):																								
			...
			try:																										#enter correct language and configuration
				for i in LANGS:
					if (i == file.language):
						if (i == 'ENG'):
							if (file.flag51 == True):
			...
			try:																										#set audio tracks to appropriate variables 
				...
				print e			
																														...
																														#### set fields to operate using correct variables
			...
			val01 = str('"'+self.titlevar.get()+'" +9TEXTED VERSION+0')													#use '+9' and '+0' so that windows reads it as '(' and ')'
			val02 = str('"'+self.titlevar.get()+'" +9TEXTLESS VERSION+0')

			self.spec1var.set("SPEC 1")
			...
			self.spec1radio1 = tk.Radiobutton(self, text='"Title" (TEXTED VERSION)',value=val01,variable=self.spec1var).grid(row=5,column=1,sticky=tk.W,padx=10)
			self.spec1radio2 = tk.Radiobutton(self, text='"Title" (TEXTLESS VERSION) ',value=val02,variable=self.spec1var).grid(row=5,column=1,sticky=tk.E,padx=10)
			self.spec1radio3 = tk.Radiobutton(self, text='THEATRICAL FEATURE',value='THEATRICAL FEATURE',variable=self.spec1var).grid(row=6,column=1,sticky=tk.W,padx=10)
			self.spec1radio4 = tk.Radiobutton(self, text='THEATRICAL TRAILER',value='THEATRICAL TRAILER',variable=self.spec1var).grid(row=6,column=1,sticky=tk.E,padx=10)
			...
			self.spec2radio1 = tk.Radiobutton(self, text='BTS / NO TEXTLESS @ TAIL',value='BTS / NO TEXTLESS @ TAIL',variable=self.spec2var).grid(row=7,column=1,sticky=tk.W,padx=10)
			self.spec2radio2 = tk.Radiobutton(self, text='BTS / TEXTLESS @ TAIL',value='BTS / TEXTLESS @ TAIL',variable=self.spec2var).grid(row=7,column=1,sticky=tk.E,padx=10)
			self.spec2radio3 = tk.Radiobutton(self, text='NO BTS / TEXTLESS @ TAIL',value='NO BTS / TEXTLESS @ TAIL',variable=self.spec2var).grid(row=8,column=1,sticky=tk.W,padx=10)
			self.spec2radio4 = tk.Radiobutton(self, text='NO BTS / NO TEXTLESS @ TAIL',value='NO BTS / NO TEXTLESS @ TAIL',variable=self.spec2var).grid(row=8,column=1,sticky=tk.E,padx=10)
			...
			self.clientIDField = tk.Entry(self, textvariable=self.clientID).grid(row=9, column=1, sticky=tk.E,padx=10)
			self.clientIDFieldLabel = tk.Label(self, text='Client ID: ').grid(row=9, column=1, sticky=tk.W,padx=10)
	...
	def quitApp(self):																									#quit application
		self.quit()

	def createQuitBUtton(self):																							#create button to quit application
