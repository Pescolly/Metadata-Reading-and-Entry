...
import PyQTMovie2 as qt
import OutputStringMaker as osm
import string
import re
import ControlNumberEntryWindow as cne

class Window(tk.Frame):
	def __init__(self, parent):
		tk.Frame.__init__(self,parent)
#		self.image = tk.PhotoImage(file='slate.gif') ####!!!!!!!!!!!!!!!TURN OFF SLATE DURING DEV!!!!!!!!!!!!!########### 
...
class Application(tk.Frame):
	def __init__(self, master=None):
		...
		self.OUTPUT_FILENAME = '/Volumes/dam/_NBCU/EntryScriptOutput/output-test.txt'
	...
	def createEntryWindows(self,file=None):
		if (file != None):
			...
			deCamelCasedList = re.split('([A-Z][a-z]*|[0-9]*)',file.title)													#use camelcasing for irony
			deCamelCasedTitle = ''
			print deCamelCasedList
			try:
				while True:
			...
			for i in deCamelCasedList:
				...
				if (i == deCamelCasedList[-1]):
					...
					deCamelCasedTitle += (str(i).upper()+' ')
			print deCamelCasedTitle
			self.titlevar.set(deCamelCasedTitle)
			...
			self.spec1radio1 = tk.Radiobutton(self, text='"Title" (TEXTED VERSION)',value='"'+self.titlevar.get()+'" +9TEXTED VERSION+0',variable=self.spec1var).grid(row=5,column=1,sticky=tk.W,padx=10)
			self.spec1radio2 = tk.Radiobutton(self, text='"Title" (TEXTLESS VERSION) ',value='"'+self.titlevar.get()+'" +9TEXTLESS VERSION+0',variable=self.spec1var).grid(row=5,column=1,sticky=tk.E,padx=10)
			...
			self.spec2radio1 = tk.Radiobutton(self, text='BTS - NO TEXTLESS @ TAIL',value='BTS - NO TEXTLESS @ TAIL',variable=self.spec2var).grid(row=6,column=1,sticky=tk.W,padx=10)
			self.spec2radio2 = tk.Radiobutton(self, text='BTS W/ TEXTLESS @ TAIL',value='BTS W/ TEXTLESS @ TAIL',variable=self.spec2var).grid(row=6,column=1,sticky=tk.E,padx=10)
			self.spec2radio3 = tk.Radiobutton(self, text='NO BTS / TEXTLESS @ TAIL',value='NO BTS / TEXTLESS @ TAIL',variable=self.spec2var).grid(row=7,column=1,sticky=tk.W,padx=10)
			self.spec2radio4 = tk.Radiobutton(self, text='NO BTS - W/ TEXTLESS @ TAIL',value='NO BTS - W/ TEXTLESS @ TAIL',variable=self.spec2var).grid(row=7,column=1,sticky=tk.E,padx=10)
